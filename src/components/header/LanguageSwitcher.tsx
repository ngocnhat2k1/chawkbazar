...
export const LanguageSwitcher = (props: Props) => {
  ...
  return (
    <div className="relative ltr:ml-2 rtl:mr-2 ltr:lg:ml-0 rtl:lg:mr-0 z-10 w-[140px] sm:w-[150px] lg:w-[130px] xl:w-[150px]">
      <button className="border border-gray-300 text-heading text-[13px] xl:text-sm font-semibold relative w-full py-2 ltr:pl-3 rtl:pr-3 ltr:pr-7 rtl:pl-7 ltr:text-left rtl:text-right bg-white rounded-lg shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-opacity-75 focus-visible:ring-white focus-visible:ring-offset-orange-300 focus-visible:ring-offset-2 focus-visible:border-indigo-500 cursor-pointer"onClick={() => setOpen(!open)}>
      ...
      {open && (
        <ul className="absolute w-full py-1 mt-1 overflow-auto bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none text-sm">
          {listLanguages.map((language, index) => (
            <li
              key={index}
              className="text-gray-900 cursor-pointer select-none relative py-2 px-3"
              id={`headlessui-listbox-option-:r${index + 1}:`}
              role="option"
              tabIndex={-1}
              aria-selected={selectedLanguage === language}
              data-headlessui-state={
                selectedLanguage === language ? 'selected' : undefined
              }
              onClick={() => {
